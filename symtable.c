...
static int hash(char *str)
{
   ...
   for (i=0; i < strlen(str); i++)
       ...
       //h += str[i];
   h = h % TABLESIZE;
   return h;           // returns the now hashed value of the string it was found in
}
...
//int addSymbol(Symbol** table, char* name, int slopeLevel, DataType type)
int addSymbol(Symbol** table, char* name, int scopeLevel, DataType type)
{
   if(table != NULL)
   {
      Symbol* symNode  = (Symbol*) malloc(sizeof(Symbol));
       
      char* symName  = strdup(name);
      int   symIndex = hash(name);
      symNode -> name       = symName;
      symNode -> scopeLevel = scopeLevel;
      symNode -> type       = type;
      symNode -> next       = table[symIndex];
      
      table[symIndex] = symNode;
      return 0;
   }
   return -1;
}
...
{
   int symIndex = hash(name);
   Symbol* cursor = table[symIndex]; 
   
   while(cursor != NULL)
   {
       if(cursor == table[symIndex])
       {
           return cursor;
       }
       cursor = cursor -> next;
